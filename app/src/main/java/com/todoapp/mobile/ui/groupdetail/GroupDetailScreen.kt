...
fun GroupDetailScreen(
    ...
    LaunchedEffect(lifecycleOwner) {
        lifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.RESUMED) {
            viewModel.onAction(GroupDetailContract.UiAction.OnScreenResumed)
        }
    }
    ...
    if (successState?.pendingDeleteTaskId != null) {
        AlertDialog(
            onDismissRequest = { viewModel.onAction(GroupDetailContract.UiAction.OnDeleteTaskDismiss) },
            ...
            confirmButton = {
                TextButton(onClick = { viewModel.onAction(GroupDetailContract.UiAction.OnDeleteTaskConfirm) }) {
                    Text(
            ...
            dismissButton = {
                TextButton(onClick = { viewModel.onAction(GroupDetailContract.UiAction.OnDeleteTaskDismiss) }) {
                    Text(
    ...
    if (successState?.pendingAssignTaskId != null) {
        AlertDialog(
            onDismissRequest = { viewModel.onAction(GroupDetailContract.UiAction.OnAssignToMeDismiss) },
            ...
            dismissButton = {
                TextButton(onClick = { viewModel.onAction(GroupDetailContract.UiAction.OnAssignToMeDismiss) }) {
                    Text(
                        text = stringResource(R.string.cancel),
                        color = TDTheme.colors.gray,
                    )
