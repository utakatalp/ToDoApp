...
constructor(
    ...
    override val languageFlow: Flow<LanguagePreference> =
        dataStoreHelper
            .getString(LanguagePreference.PREFS_KEY, LanguagePreference.ENGLISH.name)
            .map { safeValueOf(it) }
    ...
    override suspend fun saveLanguage(language: LanguagePreference) {
        ...
        context
            .getSharedPreferences(LanguagePreference.PREFS_NAME, Context.MODE_PRIVATE)
            .edit { putString(LanguagePreference.PREFS_KEY, language.name) }
    }

    override suspend fun getCurrentLanguage(): LanguagePreference = dataStoreHelper
        .getString(LanguagePreference.PREFS_KEY, LanguagePreference.ENGLISH.name)
        .first()
        .let { safeValueOf(it) }

    private fun safeValueOf(name: String): LanguagePreference = try {
